...
export function GridRow({
  ...
  const [closestDateIdx, setClosestDateIdx] = useState(
    Math.round(globalTimeScaler.getColumn(dayjs.utc(milestone.due_date.get()).toDate())),
  );
  useEffect(() => {
  ...
  const childLink = useMemo(
    ...
    [issueDataState.value, milestone, routerQuery, viewMode],
  );
  ...
  const rowItem = (
    <div
      ...
      className={`${styles.item} ${styles.issueItem} ${clickable && styles.wrapperLink} js-milestoneCard ${
        clickable && className
      }`}
    >
